...
if [ ! -d "$REPO_ROOT/phase5" ]; then
    echo "ERROR: phase5/ directory not found at $REPO_ROOT/phase5"
    exit 1
fi
...
python3 -m pip install -r "$PHASE5_DIR/requirements.txt"
...
python3 -c "
import sys
sys.path.insert(0, '$REPO_ROOT/phase5')
...
python3 "$PHASE5_DIR/db/migrate_from_json.py" \
    --input "$JSON_FILE" \
...
from phase5.api.db import verify_database_connectivity
...
echo "   cd $REPO_ROOT"
echo "   python3 -m uvicorn phase5.api.main:app --reload"
echo ""
...
echo ""
echo "4. Run tests:"
echo "   pytest phase5/tests/test_phase5_integration.py -v"
echo ""
