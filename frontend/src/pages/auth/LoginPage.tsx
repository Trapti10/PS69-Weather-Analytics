...
/**
 * Login errors are shown using the backend's own `detail` text when present
 * (e.g. "Invalid email or password", or the role-mismatch message from
 * routes/auth.py:login) rather than the app-wide normalizeApiError()
 * messages. That shared helper's generic 401/403 copy ("Your session has
 * expired...", "You don't have permission...") is correct for actions
 * taken *after* being logged in, but wrong here - there is no session to
 * expire on a failed login, and the specific "wrong login type" message is
 * exactly what a person needs to see to pick the right dropdown option.
 * Falls back to the normalized message for anything without a plain-string
 * detail (e.g. a network error).
 */
